    ...
    <div className="w-full h-full bg-black pb-12">
      <div>
        <div>
          ...
          <p className="ml-8 text-white text-[24px]">
            {currentDate} Updated Items
          </p>
          <div className="flex mt-2 mx-4 items-center text-center justify-center">
        ...
        <div className="place-items-center grid grid-cols-2 min-[600px]:grid-cols-3 min-[900px]:grid-cols-4 lg:grid-cols-5 min-[1300px]:grid-cols-6 2xl:grid-cols-8">
          {Figures.map((figure: FigureData) => {
            return (
              <Link key={figure.id} to={`/figure/${figure.id}`}>
                <Card
                  key={figure.title}
                  title={truncateString(figure.title, 41)}
                  image={figure.image}
                  price={figure.price}
      ...
      <div>
        <div>
          <p className="ml-8 text-white text-[24px] mt-8">
            {yesterdayDate} Updated Items
          </p>
          <div className="flex mt-2 mx-4 items-center text-center justify-center">
            <div className="flex-1 border-t-2 border-white"></div>
          </div>
        ...
        <div className="place-items-center grid grid-cols-2 min-[600px]:grid-cols-3 min-[900px]:grid-cols-4 lg:grid-cols-5 min-[1300px]:grid-cols-6 2xl:grid-cols-8">
          {[...Figures].reverse().map((figure: FigureData) => {
            return (
              <Link key={figure.id} to={`/figure/${figure.id}`}>
                <Card
                  key={figure.title}
