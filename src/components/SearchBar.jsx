...
export default function SearchBar() {
    ...
    const handleSearch = (e) => {
        ...
        const filteredFigures = Figures.filter(item => item.title.toLowerCase().includes(searchTerm)).slice(0,6);
  ...
  return (
    ...
    <CiSearch size={24} className='absolute top-0 left- cursor-pointer text-white'/>
</div>
      <input
        ...
        activeSearch.length > 0 && (
            <div className='absolute top-20 p-4 bg-white xl:w-[800px] md:w-[400px] w-[200px]'>
                {activeSearch.map(item => (
                    <Link key={item.id} onClick={(() => handleClick(item.title))} to={`/figure/${item.id}`}>
